...
export async function startWhatsAppConnection(
  ...
  // Use pairing code instead of QR if PHONE_NUMBER env var is set
  const pairingPhone = process.env.PHONE_NUMBER;
  ...
  const sock = makeWASocket({
    ...
    // Disable QR printing when using pairing code
  ...
  if (pairingPhone && !state.creds.registered) {
    ...
    setTimeout(async () => {
      try {
        ...
        console.error(`\n*** PAIRING CODE: ${code} ***`);
        console.error(`Enter this code in WhatsApp > Linked Devices > Link a Device > Link with phone number\n`);
      } catch (err: any) {
